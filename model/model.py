...
class DenseBlock(nn.Module):
    ...
    def forward(self, x):  # x.shape(1,2,256,256)
        x1 = self.lrelu(self.conv1(x))
        x2 = self.lrelu(self.conv2(torch.cat((x, x1), 1)))
        x3 = self.lrelu(self.conv3(torch.cat((x, x1, x2), 1)))
        x4 = self.lrelu(self.conv4(torch.cat((x, x1, x2, x3), 1))) # x1, x2, x3, x4.shape(1,32,256,256)
        x5 = self.conv5(torch.cat((x, x1, x2, x3, x4), 1)) # x5.shape(1,1,256,256)
...
class InvBlock(nn.Module):
    ...
    def forward(self, x, rev=False):
        if not rev:            
            # invert1x1conv 
            x, logdet = self.flow_permutation(x, logdet=0, rev=False) # x.shape: (1,3,256,256);  logdet (scalar)
            
            # split to 1 channel and 2 channel. 
            x1, x2 = (x.narrow(1, 0, self.split_len1), x.narrow(1, self.split_len1, self.split_len2)) # x1.shape(1,1,256,256); x2.shape(1,2,256,256)

            y1 = x1 + self.F(x2) # 1 channel; y1.shape(1,1,256,256)
            self.s = self.clamp * (torch.sigmoid(self.H(y1)) * 2 - 1) # s.shape(1,2,256,256)
            y2 = x2.mul(torch.exp(self.s)) + self.G(y1) # 2 channel; self.G(y1).shape(1,2,256,256) => y2.shape(1,2,256,256)
