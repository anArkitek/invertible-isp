...
class InvertibleConv1x1(nn.Module):
    def __init__(self, num_channels, LU_decomposed):
        ...
        if not LU_decomposed:
            ...
            p, lower, upper = torch.lu_unpack(*torch.linalg.lu_factor(w_init)) # p: permutation matrix; lower: lower triangle matrix; upper: upper triangle matrix
            s = torch.diag(upper)
            sign_s = torch.sign(s)
            log_s = torch.log(torch.abs(s))
            upper = torch.triu(upper, 1) # remove diagonal elements
            l_mask = torch.tril(torch.ones(w_shape), -1) # tensor([[0., 0., 0.], [1., 0., 0.], [1., 1., 0.]])
            eye = torch.eye(*w_shape)

            self.register_buffer("p", p) # fix permutation matrix
            self.register_buffer("sign_s", sign_s) # fix signs of diagonal elements; sin_s.shape(3)
            self.lower = nn.Parameter(lower)  # lower.shape(3,3)
            self.log_s = nn.Parameter(log_s)  # log_s.shape(3)
            self.upper = nn.Parameter(upper)  # upper.shape(3,3)
            self.l_mask = l_mask  # l_mask.shape(3,3); req_grad=False
            self.eye = eye;  # eye.shape(3,3); req_grad=False
    ...
    def get_weight(self, input, reverse):
        ...
        if not self.LU_decomposed:
            ...
            u = self.upper * self.l_mask.transpose(0, 1).contiguous()  # get the upper triangle matrix
    ...
    def forward(self, input, logdet=None, reverse=False):
        """
        log-det = log|abs(|W|)| * num_of_pixels
        """
        weight, dlogdet = self.get_weight(input, reverse)
        # weight.shape: (3,3,1,1);  dlogdet (scalar)
        if not reverse:
            z = F.conv2d(input, weight) # input.shape: (1,3,256,256) z.shape: (1,3,256,256)
            if logdet is not None:
                logdet = logdet + dlogdet
            return z, logdet
        else:
